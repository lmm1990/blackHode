...
public class InsertToDbJob {
    ...
    public void run() {
        ...
        if (DataHandler.countDataList.get(baseVersion).isEmpty()) {
            clearOldVersionData(baseVersion);
     ...
     * @param version 版本号
     */
    private void clearOldVersionData(int version) {
        //清空已插入数据表的数据
        DataHandler.countDataList.get(version).clear();
    }
    ...
    private void stopRuning() {
        while (true) {
            ...
            //数据已入库&kafka消费数量==已统计数量，程序停止
            if (MonitorDataHandler.kafkaConsumeCount.getOrDefault(day, new AtomicLong()).get() ==
